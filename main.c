...
// Submit the source code of the kernel to OpenCL
	program = clCreateProgramWithSource(context, 1,
			(const char **) &app_kernel.src, 0, &ret);
	if (ret == CL_SUCCESS)
// and compile it (after this we could extract the compiled version)
	...
	if (ret < 0)
	{
		...
		clGetProgramBuildInfo(program, device_id, CL_PROGRAM_BUILD_LOG,
				app_kernel.size, app_kernel.src, NULL);
		printf("\n%s", app_kernel.src);
	}
	assert(ret == CL_SUCCESS);
	buffer_input = clCreateBuffer(context, CL_MEM_READ_ONLY, size_2d, NULL,
			&ret);
	assert(ret == CL_SUCCESS);
	buffer_output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, size_2d, NULL,
			&ret);
	assert(ret == CL_SUCCESS);
	...
	ret = clEnqueueWriteBuffer(cq, buffer_input, CL_TRUE, 0, size_2d, data, 0,
			NULL, NULL);
	assert(ret == CL_SUCCESS);
	ret = clEnqueueNDRangeKernel(cq, kernel, dimension, NULL, &global[0],
			&local[0], 0, NULL, NULL);
	assert(ret == CL_SUCCESS);
	ret = clEnqueueReadBuffer(cq, buffer_output, CL_TRUE, 0, size_2d, data2, 0,
			NULL, NULL);
	assert(ret == CL_SUCCESS);
	printf("\nResult:\n");
	for (i = 0; i < buffer_width; i++)
